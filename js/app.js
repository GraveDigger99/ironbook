(() => {
  ...
  function rangeInit() {
          ...
          itemTotal.addEventListener('keydown', function (e) {
            ...
            switch (e.which) {
              ...
              case 38:
                ...
                if (position !== null)
                  itemTotal.noUiSlider.setHandle(handle, value + position);
                break;
    ...
    if (totalItems2.length)
      totalItems2.forEach((totalItem) => {
  ...
  function initSliders() {
    if (document.querySelector('.configurator__slider'))
      new core('.configurator__slider', {
    ...
    if (document.querySelector('.detail-info__slider'))
      new core('.detail-info__slider', {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleSelectButtonClick(event) {
      ...
      const selectButton = configurator.querySelector(
        '.slide-configurator__select-button'
      );
      console.log(selectButton);
      const clearButton = configurator.querySelector(
        '.slide-configurator__clear-button'
      );
      console.log(clearButton);
      const actionsBlock = configurator.querySelector(
        '.slide-configurator__actions'
      );
      ...
      if (!configurator.classList.contains('clicked')) {
        ...
        actionsBlock.classList.add('hovered');
      }
    }
    function handleClearButtonClick(event) {
      ...
      actionsBlock.classList.remove('hovered');
    }
    configurators.forEach(function (configurator) {
      ...
      else configurator.addEventListener('click', handleSelectButtonClick);
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (pinWrapper) {
      ...
      var configuratorInner = document.querySelector(
        '.body-configurator__inner'
      );
      ...
      function handleScroll() {
        if (window.innerWidth >= 767.98) {
          ...
          var configuratorInnerBottom =
            configuratorInner.offsetTop + configuratorInner.offsetHeight;
          var scrollPosition = window.scrollY;
          if (
            scrollPosition >= pinBlockOffset &&
            scrollPosition + (pinBlock.offsetHeight + 160) >
              configuratorInnerBottom
          ) {
            pinBlock.classList.remove('pinned');
            pinBlock.style.position = 'absolute';
            pinBlock.style.bottom = '0';
            placeholder.style.height = '0';
          } else if (
            scrollPosition >= pinBlockOffset &&
            scrollPosition + pinBlock.offsetHeight <= configuratorInnerBottom
          ) {
            pinBlock.classList.add('pinned');
            pinBlock.style.position = 'fixed';
            pinBlock.style.paddingBottom = '0';
  ...
  function initComputerBuilder() {
    ...
    const tasks = {
      0: {
        text: 'Добавьте комплектующие',
        color: '#fe3838',
      },
      25: {
        text: 'Минимальная производительность',
        color: '#fe3838',
      },
      50: {
        text: 'Подходит для дома и офиса',
        color: '#ea9a3c',
      },
      100: {
        text: 'Подходит для игр и работы с медиа',
        color: '#78b95d',
      },
    };
    let totalProgress = 0;
    let collectButtonClicked = false;
    document.querySelectorAll('.progress-value').forEach(function (item) {
      item.textContent = tasks[0].text || '';
    });
    function calculatePerformance(selectedComponents) {
      let performance = 0;
      selectedComponents.forEach((component) => {
        const componentName = component.getAttribute('data-component');
        const componentWeight = componentConfig[componentName]?.weight || 0;
        performance += componentWeight;
      });
      return Math.min(performance, 100);
    }
    function updatePerformance() {
      const selectedComponents = Array.from(
        document.querySelectorAll('.component.selected')
      );
      const performance = calculatePerformance(selectedComponents);
      totalProgress = performance;
      updateLinearProgressBar();
    }
    function toggleComponent(component) {
      const addButton = component.querySelector('.button-add');
      const closeButton = component.querySelector('.delete-btn');
    ...
    function showProgressContainer() {
      ...
      document.querySelector('.aside-configurator__result').style.display =
        'none';
      document.documentElement.style.setProperty('--progress', totalProgress);
      let updatedText = '';
      let updatedColor = '';
      for (const taskPercentage in tasks)
        if (totalProgress >= parseFloat(taskPercentage)) {
          updatedText = tasks[taskPercentage].text;
          updatedColor = tasks[taskPercentage].color;
        }
      document.querySelectorAll('.progress-value').forEach(function (item) {
      ...
      document
        ...
        .style.setProperty('--progress-color', updatedColor);
      updateLinearProgressBar();
    }
    ...
    function updateLinearProgressBar() {
      if (collectButtonClicked) {
        ...
        if (totalProgress === 0) {
          linearProgressBar.style.width = '32px';
          linearProgressBar.style.height = '8px';
          linearProgressBar.style.backgroundColor = '#fe3838';
        } else {
          linearProgressBar.style.width = `${totalProgress}%`;
          linearProgressBar.style.backgroundColor =
            getLinearProgressBarColor(totalProgress);
        }
      }
    }
    function getLinearProgressBarColor(progress) {
      if (collectButtonClicked)
        if (progress >= 100) return '#78b95d';
        else if (progress >= 50) return '#ea9a3c';
        else return '#fe3838';
      return '';
    }
    document
      .getElementById('componentList')
